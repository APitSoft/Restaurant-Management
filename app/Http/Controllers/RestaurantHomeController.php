...
class RestaurantHomeController extends Controller
{
    ...
    public  function  addstaff(Request $request){
        ...
        $Waiter_a = $request->Waiter;

        if ($Waiter_a == "Waiter") {
            $Waiter = 'Waiter';
        }elseif ($Waiter_a == "Kitchen"){
            $Waiter = 'Kitchen';
        }
        else {
            $Waiter = 'Manager';
    ...
    public function serveGroup(){

        return view('pages.restaurant.Serve-Group.Add-serve-Group');
    }
    public function AddserveGroup(Request $request){
        $res_id = Auth::guard('restaurantUser')->user()->restaurant_id;
...
//        $data['password']= bcrypt($request->password);
        ...
        DB::table('tbl_restaurants_group')
            ->insert($data);
        toastr()->success('Add Serve Group successfully.');
    return redirect('restaurant/View-serve-Group');
        //return back();
    }
    public function ViewserveGroup(){
        $res_id = Auth::guard('restaurantUser')->user()->restaurant_id;
        $Show=DB::table('tbl_restaurants_group')
            ->where('restaurant_id',$res_id)
            ->get();
        return view('pages.restaurant.Serve-Group.Show-serve-Group',compact('Show'));
    }
    public function EditServeGroup($id){

        $Show=DB::table('tbl_restaurants_group')
            ->where('id',$id)
            ->first();
        return view('pages.restaurant.Serve-Group.Edit-serve-Group',compact('Show'));
    }
    public function updateServeGroup(Request $request){
        $id=$request->id;
        $res_id = Auth::guard('restaurantUser')->user()->restaurant_id;
        $data=array();
        $data['group_name']=$request->group_name;
...
//        $data['password']= bcrypt($request->password);
        ...
        DB::table('tbl_restaurants_group')
            ->where('id',$id)
            ->update($data);
        toastr()->success('Update Serve Group successfully.');
        return redirect('restaurant/View-serve-Group');
      //  return back();
    }

    public function DeleteServeGroup($id){
        DB::table('tbl_restaurants_group')
            ->where('id',$id)
            ->delete();
        toastr()->success('Delete Serve Group successfully.');
        return back();
    }
    public function AddWaiterGroup($id){
        $res_id = Auth::guard('restaurantUser')->user()->restaurant_id;
        $Show=DB::table('tbl_restaurant_users')
            ->where('role','=','Waiter')
            ->where('restaurant_id',$res_id)
            ->get();

        $waiter= DB::table('tbl_restaurants_group_waiter')
            ->join('tbl_restaurant_users','tbl_restaurants_group_waiter.res_wit_id','=','tbl_restaurant_users.id')
            ->join('tbl_restaurants_group','tbl_restaurants_group_waiter.restaurants_group_id','=','tbl_restaurants_group.id')
            ->select('tbl_restaurants_group_waiter.*','tbl_restaurant_users.manager_name','tbl_restaurants_group.group_name')
            ->where('tbl_restaurants_group_waiter.restaurants_group_id',$id)
            ->get();
       // dd($waiter);

        return view('pages.restaurant.Serve-Group.Add-Waiter-Group',compact('Show','waiter','id'));
    }
    public function AddWaiterGroupnew(Request $request){

        $res_id = Auth::guard('restaurantUser')->user()->restaurant_id;

        $data=array();
        $data['res_wit_id']=$request->waiter;
        $data['restaurants_group_id']=$request->id;
        $data['restaurant_id']=$res_id;

        DB::table('tbl_restaurants_group_waiter')
            ->insert($data);
        return back();
    }
    public function DeleteWaiterall($id){
