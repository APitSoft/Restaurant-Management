...
function color(id)
{
    //$("#color_code").val('')
    // $.ajax({
    //     url: "../../search_reservation/" + id,
    //     method: "GET",
    //     success: function (response) {
    //         if(response == true)
    //         {
    //             //sessionStorage.setItem("color", "#f70909");
    //             _color = '#f70909';
    //             console.log(_color);
    //             //alert('1')
    //             // $("#color_code").val('#f70909')
    //         }
    //         else
    //         {
    //
    //             //alert('2')
    //             //sessionStorage.setItem("color", "#2caccc");
    //             _color = '#2caccc';
    //             console.log(_color);
    //             // $("#color_code").val('#2caccc')
    //         }
    //         return _color;
    //
    //     }
    ...
    $('#add_to_cart').on('click', function () {
        ...
        $('.modal_modifiers[data-selected=selected]').each(function (i, obj) {
            if (j == $('.modal_modifiers[data-selected=selected]').length) {
                ...
                selected_modifiers += $(this).find('p').html() + '<br>';
                selected_modifiers_id += $(this).attr('id').substr(9) + '<br>';
                selected_modifiers_price += $(this).find('.modifier_price').html() + '<br>';
            }
            j++;
        });
        ...
        //get modifiers price
        var modifiers_price = parseFloat($('#modal_modifier_price_variable').html()+'<br>').toFixed(2);
        ...
        if (selected_modifiers != "") {
            ...
            draw_table_for_order += '<span id="item_modifiers_id_table_' + item_id + '" style="display:none;">' + selected_modifiers_id + '</span>';
            draw_table_for_order += '<span id="item_modifiers_price_table_' + item_id + '" style="display:none;">' + selected_modifiers_price + '</span>';
            draw_table_for_order += '<div style="padding-left: 20px;" class="single_order_column first_column fix"><span id="item_modifiers_table_' + item_id + '">' + selected_modifiers + '</span></div>';
            draw_table_for_order += '<div style="float: right;margin-right: 25px;">' + selected_modifiers_price + '</div>';
            draw_table_for_order += '<div class="single_order_column fifth_column fix" style="float: right">' + currency + ' <span id="item_modifiers_price_table_' + item_id + '">' + modifiers_price + '</span></div>';
            draw_table_for_order += '</div>';
        }
        if (note != "") {
            draw_table_for_order += '<div class="third_portion fix">';
...
function set_all_items_modifiers_amount() {
    $('.order_holder .single_order').each(function (i, obj) {
        ...
        if ($(this).find('#item_modifiers_price_table_' + item_id).length > 0) {
            ...
            var modifiers_price_array = (comma_separeted_modifiers_price != "") ? comma_separeted_modifiers_price.split("<br>") : '';
            modifiers_price_array.forEach(function (modifier_price) {
                modifiers_price = (parseFloat(modifiers_price) + parseFloat(modifier_price)).toFixed(2);
            });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var key in response) {
                ...
                var table_name = (response[key].table!= null) ? response[key].table.name : "";
                var waiter_name = (response[key].waiter != null) ? response[key].waiter.manager_name : "";
                ...
                if (response[key].table_id != null) {
                    ...
                    // 	w++;
                    // }
                    $tables_booked = response[key].table!=null?response[key].table.name:'';
                } else {
                    tables_booked = 'None';
                }
           ...
           // console.log(response[key].sale_id);
    ...
    for (var key in response.sales_details) {
        ...
        for (var mod_key in this_item.modifiers) {
            ...
            if (i == total_modifier) {
                ...
                selected_modifiers += this_modifier.name + '<br>';
                selected_modifiers_id += this_modifier.modifier_id + '<br>';
                selected_modifiers_price += this_modifier.modifier_price + '<br>';
                modifiers_price = (parseFloat(modifiers_price) + parseFloat(this_modifier.modifier_price)).toFixed(2);
            }
            i++;
        }
        ...
        if (selected_modifiers != "") {
            ...
            draw_table_for_order += '<div class="single_order_column first_column fix"><span id="item_modifiers_table_' + this_item.food_menu_id + '">'+  selected_modifiers +  '</span></div>';
            draw_table_for_order += '<div style="float: right;margin-right: 25px;">' + selected_modifiers_price + '</div>';
            draw_table_for_order += '<div style="float: right;" class="single_order_column fifth_column fix">' + currency + ' <span id="item_modifiers_price_table_' + this_item.food_menu_id + '">' + modifiers_price + '</span></div>';
            draw_table_for_order += '</div>';
        }
        if (this_item.menu_note != "") {
            draw_table_for_order += '<div class="third_portion fix">';
