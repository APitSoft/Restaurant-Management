...
<div class="wrapper fix">
    <div class="top_header_part fix">
        ...
        <div class="header_part_left fix">
            ...
            <button id="help_button"><i class="fas fa-question-circle"></i> Read Before Begin</button>
...
<div id="finalize_order_modal" class="modal">
    ...
    <!-- Modal content -->
    <div class="modal-content" style=" width: 400px !important;" id="modal_finalize_order_details">
...
{{--                    @csrf--}}
{{--                    <button id="gift_add" type="button" onclick="saveData()" class="btn btn-primary">ADD</button>--}}
{{--                </form>--}}
{{--            </div>--}}
            <button onclick="myFunction2()"  class="btn btn-dark"  type="button">Add Payment</button>
        </div>
        <div class="fo_3 fix " id="gift_card">
            <div class="half fix floatleft textleft">Gift Card</div>
            <div class="half fix floatleft textright"><input type="text" name="payment_gift_card_no_modal_input" id="payment_gift_card_no_modal_input"></div>
            <div class="" id="gift_card_checking_response">
                <input type="text"  name="pay_amount_invoice_modal_input"  id="pay_amount_invoice_input">
            </div>
        </div>
        <div class="fo_3 fix">
            <table id="myTable">
                <thead>
                    <tr>
                        <th><div class="half fix floatleft textleft">Pay Amount</div></th>
                        <th><div class="half fix floatleft textright">Due Amount</div></th>
                        <th><div class="half fix floatleft textright">Action</div></th>
                    </tr>
                </thead>
                <tr>
                    <td>
                        <div class="half fix floatleft textleft"><input type="text"  name="pay_amount_invoice_modal_input"  id="pay_amount_invoice_input" disabled></div>
                    </td>
                    <td>
...
<script type="text/javascript">
    ...
    function myFunction2() {
        ...
        cell1.innerHTML = '<input type="text"  name="pay_amount_invoice_input" id="dynpay[]"  onkeyup="calculator();">';
var options ="";
<?php  foreach($allmethod as $all_pay) :?>
options += '<option value="<?php echo $all_pay->id; ?>"><?php echo $all_pay->Methord; ?></option>';
<?php endforeach   ?>
        cell2.innerHTML = '<select name="finalie_order_payment_method" class="from-control" ><option value="">Payment Method</option> '+options +'   </select>';
        cell3.innerHTML = "<button onclick=\"deleteRow(this)\" type=\"button\" class=\"btn btn-danger\">Delete</button>";
        i++;
    }
var total =0;
    function  calculator()
    {
       // total += parseFloat(value);
        ...
        // unitprice =  $('#new_pay').val();

        // var quantity = 0.00;

        // quantity =  $('#new_amount').val();

        // var total = 0.00;

        // total = unitprice - quantity;
       // $('#pay_amount_invoice_input').val();
        //$('#pay_amount_invoice_input').val(total-1);
        total =0;
$('[id^=dynpay]').each(function(idx, item){
    total+=parseFloat(this.value);

});
//alert(total);

$('#pay_amount_invoice_input').val(total);  //get

    }

    {{--function saveData(){--}}
    {{--    var gift_add = $("#gift_add").val();--}}
    {{--    var request;--}}
    {{--    request = $.ajax({--}}
    {{--        url: "{{route('saveData')}}",--}}
    {{--        type: "post",--}}
